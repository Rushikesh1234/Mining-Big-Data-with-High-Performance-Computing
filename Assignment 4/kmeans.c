...
void printInputData(float* all_vector, const int num_elements) {
  for (int i = 0; i < num_elements; i++) {
    printf("%f ", all_vector[i]);
  }
}
...
int main(int argc, char** argv) {
  ...
  if (rank == 0) {
    ...
    printf("\nInput Data: \n");
    printInputData(all_vector, d * vectors_per_process * nprocs);
  ...
  while (norm > 0.00001) { 
    ...
    if (rank == 0) {
      ...
      // Perform Normalization for calculating centroids
      norm = distance2(total_sums, centroids, d*k);

      printf("\nNormalization value: %f\n",norm);

      // In centroids, insert fresh centroids from above calculated normalisation.
      for (int i=0; i<k*d; i++) {
